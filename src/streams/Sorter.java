    ...
    public long sort(Element[] elements) {
        Long[] sums = Arrays.stream(elements)
            .map(e -> {
                ...
                int[] numbers = Arrays.stream(e.getElement().getNumbers()).distinct().toArray();
                e.getElement().setNumbers(numbers);
                return e;
            }).map(e -> {
                int[] numbers = Arrays.stream(e.getElement().getNumbers()).filter(i -> i < e.getAverage()).toArray();
                e.getElement().setNumbers(numbers);
                return e;
            }).map(e -> Arrays.stream(e.getElement().getNumbers()).asLongStream().sum()).toArray(Long[]::new);

        return Arrays.stream(sums).mapToLong(Long::longValue).sum() / sums.length;
    }
